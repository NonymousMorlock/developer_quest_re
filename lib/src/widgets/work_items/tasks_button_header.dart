import '../../../src/game_screen/add_task_button.dart';
import '../../../src/game_screen/bug_picker_modal.dart';
import '../../../src/game_screen/project_picker_modal.dart';
import '../../../src/game_screen/team_picker_modal.dart';
...
class TasksButtonHeader extends SliverPersistentHeaderDelegate {
  ...
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) {
    return Consumer<TaskPool>(
        builder: (_, tPool, __) => Padding(
            ...
            child: AddTaskButton(
              ...
              onPressed: () async {
                var project = await showModalBottomSheet<TaskBlueprint>(
                  context: context,
                  builder: (context) => const ProjectPickerModal(),
                );
                if (project != null) {
                  var task = tPool.startTask(project);
            ...
            child: AddTaskButton(
                ...
                onPressed: () async {
                  var bug = await showModalBottomSheet<Bug>(
                    context: context,
                    builder: (context) => const BugPickerModal(),
                  );
                  if (bug != null) {
                    tPool.addWorkItem(bug);
